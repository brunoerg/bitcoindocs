...
terminated by a `0x00` byte [^1]. Each key-value pair must have a unique
key within its scope; duplicates are not allowed. The format of a record
is as follows:
...
-   Type: Proprietary Use Type `PSBT_GLOBAL_PROPRIETARY = 0xFC`
    -   Key: Variable length identifier prefix, followed by a subtype,
        ...
        -   `{0xFC}|``<prefix>`{=html}`|{subtype}|{key data}`
...
-   Type: Witness UTXO `PSBT_IN_WITNESS_UTXO = 0x01`
    ...
    -   Value: The entire transaction output in network serialization
        ...
        -   `{serialized transaction output({output value}|``<scriptPubKey>`{=html}`)}`
...
## Appendix A: Data types and their specifications {#appendix_a_data_types_and_their_specifications}
...
here
  ...
  -------- ------------- ----------------------------- ------------------------------------------
  Global   0             PSBT_GLOBAL_UNSIGNED_TX       BIP 174
  Global   1             PSBT_GLOBAL_XPUB              BIP 174
  Global   251           PSBT_GLOBAL_VERSION           BIP 174
  Global   252           PSBT_GLOBAL_PROPRIETARY       BIP 174
  Input    0             PSBT_IN_NON_WITNESS_UTXO      BIP 174
  Input    1             PSBT_IN_WITNESS_UTXO          BIP 174
  Input    2             PSBT_IN_PARTIAL_SIG           BIP 174
  Input    3             PSBT_IN_SIGHASH_TYPE          BIP 174
  Input    4             PSBT_IN_REDEEM_SCRIPT         BIP 174
  Input    5             PSBT_IN_WITNESS_SCRIPT        BIP 174
  Input    6             PSBT_IN_BIP32_DERIVATION      BIP 174
  Input    7             PSBT_IN_FINAL_SCRIPTSIG       BIP 174
  Input    8             PSBT_IN_FINAL_SCRIPTWITNESS   BIP 174
  Input    9             PSBT_IN_POR_COMMITMENT        [BIP 127](bip-0127.mediawiki "wikilink")
  Input    10            PSBT_IN_RIPEMD160             BIP 174
  Input    11            PSBT_IN_SHA256                BIP 174
  Input    12            PSBT_IN_HASH160               BIP 174
  Input    13            PSBT_IN_HASH256               BIP 174
  Input    252           PSBT_IN_PROPRIETARY           BIP 174
  ...
  Output   252           PSBT_OUT_PROPRIETARY          BIP 174
...
[^4]: **Why use 4 bytes for psbt?** The transaction format needed to
    start with a 5 byte header which uniquely identifies it. The first
    bytes were chosen to be the ASCII for psbt because that stands for
    Partially Signed Bitcoin Transaction.

[^5]: **Why Use a separator after the magic bytes?** The separator is
...
[^6]: **Why are all separators required?** The separators are required
    so that the unserializer knows which input it is unserializing data
    for.
